...
async def authorize(request: Request) -> Response:
    ...
    if (
        public_registration
        and not storage.client_exists(client_id)
        and urlparse(client_id).scheme == "https"
    ):
        metadata = await run_in_threadpool(_fetch_client_metadata_document, client_id, allow_private_cimd)
        if metadata is None:
        ...
        if (
            ...
            or auth_method not in _SUPPORTED_CIMD_AUTH_METHODS
        ):
